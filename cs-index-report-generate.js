...
const csIndexReturnFetcher = require('./cs-index-return-fetcher.js');
...
async function checkAndFetchData() {
    ...
    try {
        console.log('调用 cs-index-return-fetcher.js 的 main 函数...');
        if (typeof csIndexReturnFetcher.main === 'function') {
            await csIndexReturnFetcher.main();
        } else if (typeof csIndexReturnFetcher === 'function') {
            await csIndexReturnFetcher();
        } else {
            throw new Error('cs-index-return-fetcher.js 未导出 main 函数');
        }
        // 检查获取后的文件是否存在
        if (fs.existsSync(todayDataFile)) {
            console.log('数据获取成功');
...
async function loadJsonData() {
    try {
        // 首先检查并获取数据
        const dataFile = await checkAndFetchData();
        if (!dataFile) {
